...
CREATE TABLE `students` (
  ...
  `student_no` VARCHAR(32) UNIQUE NOT NULL COMMENT '学员编号',
  `name` VARCHAR(32) NOT NULL,
  `avatar` VARCHAR(255) DEFAULT '👨‍🎓',
  `phone` VARCHAR(20) NOT NULL,
  `password` VARCHAR(64) NOT NULL COMMENT '密码明文(演示)',
...
CREATE TABLE `coaches` (
  ...
  `experience` TINYINT DEFAULT 5 COMMENT '教龄年数',
  ...
  `rest_day` TINYINT DEFAULT 1 COMMENT '周几休息 0-周日 ... 6-周六',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `coach_ratings` (
  ...
  CONSTRAINT `fk_ratings_coach` FOREIGN KEY (`coach_id`) REFERENCES `coaches`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `courses` (
  ...
  `time_slot_name` VARCHAR(32) DEFAULT '',
  `time_period` VARCHAR(16) DEFAULT '',
  ...
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT `fk_course_student` FOREIGN KEY (`student_id`) REFERENCES `students`(`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_course_coach` FOREIGN KEY (`coach_id`) REFERENCES `coaches`(`id`) ON DELETE CASCADE,
  INDEX `idx_student_date` (`student_id`, `course_date`),
  INDEX `idx_coach_date` (`coach_id`, `course_date`),
  UNIQUE KEY `uk_coach_datetime_slot` (`coach_id`, `course_date`, `time_slot_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `reviews` (
  ...
  `tags` VARCHAR(255) DEFAULT '' COMMENT '逗号分隔',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT `fk_review_coach` FOREIGN KEY (`coach_id`) REFERENCES `coaches`(`id`) ON DELETE CASCADE,
  ...
  UNIQUE KEY `uk_course_review` (`course_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `simulator_bookings` (
  ...
  `status` VARCHAR(16) NOT NULL DEFAULT 'upcoming' COMMENT 'upcoming/completed/cancelled',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT `fk_sim_booking_student` FOREIGN KEY (`student_id`) REFERENCES `students`(`id`) ON DELETE CASCADE,
  UNIQUE KEY `uk_student_datetime` (`student_id`, `session_date`, `slot_id`),
...
CREATE TABLE `simulator_info` (
  ...
  `location` VARCHAR(128) NOT NULL,
  `notice` TEXT COMMENT '竖线分隔注意事项',
  `faq` TEXT COMMENT '格式: 问题1|回答1||问题2|回答2'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `subject_progress` (`student_id`, `subject`, `subject_name`, `full_name`, `status`, `icon`, `training_hours`, `required_hours`, `pass_date`, `score`, `color`) VALUES
(1, 1, '科一', '理论知识', 'passed', '📚', 0, 0, '2024-04-10', 96, 'linear-gradient(135deg, #07c160 0%, #10b981 100%)'),
(1, 2, '科二', '场地驾驶', 'training', '🚗', 8.5, 16, NULL, NULL, 'linear-gradient(135deg, #1989fa 0%, #3da5ff 100%)'),
(1, 3, '科三', '道路驾驶', 'not_started', '🛣️', 0, 16, NULL, NULL, 'linear-gradient(135deg, #969799 0%, #b0b1b3 100%)'),
(1, 4, '科四', '文明驾驶', 'not_started', '🎯', 0, 0, NULL, NULL, 'linear-gradient(135deg, #969799 0%, #b0b1b3 100%)');
...
-- 课程数据：使用动态日期
INSERT INTO `courses` (`student_id`, `coach_id`, `coach_name`, `coach_avatar`, `course_date`, `time_slot_id`, `time_start`, `time_end`, `time_slot_name`, `time_period`, `location`, `subject`, `hours`, `status`, `message`, `reviewed`) VALUES
...
-- 模拟器时段：7天 × 4时段
DROP PROCEDURE IF EXISTS init_simulator_sessions;
DELIMITER //
CREATE PROCEDURE init_simulator_sessions()
BEGIN
  ...
  WHILE i <= 7 DO
    ...
    -- s1 09:00-11:00
    INSERT INTO `simulator_sessions` VALUES (NULL, d, 's1', '09:00', '11:00',
      CASE WHEN i=2 THEN 'full' ELSE 'available' END, 180, 6, CASE WHEN i=2 THEN 6 ELSE 0 END);
    -- s2 13:00-15:00
    INSERT INTO `simulator_sessions` VALUES (NULL, d, 's2', '13:00', '15:00',
      CASE WHEN i=5 THEN 'rest' ELSE 'available' END, 180, 6, 0);
    -- s3 15:30-17:30
    INSERT INTO `simulator_sessions` VALUES (NULL, d, 's3', '15:30', '17:30',
      CASE WHEN i IN (1,3) THEN 'full' ELSE 'available' END, 180, 6, CASE WHEN i IN (1,3) THEN 6 ELSE 0 END);
    -- s4 18:00-20:00
    INSERT INTO `simulator_sessions` VALUES (NULL, d, 's4', '18:00', '20:00',
      CASE WHEN i=3 THEN 'rest' ELSE 'available' END, 200, 6, 0);
    SET i = i + 1;
  END WHILE;
END //
DELIMITER ;
CALL init_simulator_sessions();
DROP PROCEDURE IF EXISTS init_simulator_sessions;
